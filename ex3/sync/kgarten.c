...
struct kgarten_struct {
	 ...
	 */
	 int space;
	 pthread_cond_t child_in, teacher_out;
	/* ... */
...
void child_enter(struct thread_info_struct *thr)
{
	...
	while ( thr->kg->space == 0) {
		pthread_cond_wait(&thr->kg->child_in, &thr->kg->mutex);
		//pthread_cond_signal(&thr->kg->cond1);
	}
	--(thr->kg->space);
	++(thr->kg->vc);
...
int main(int argc, char *argv[])
{
	...
	kg->ratio = ratio;
	kg->space = 0;
	ret = pthread_cond_init(&kg->child_in, NULL);
	if (ret) {
		perror_pthread(ret, "pthread_cond_init");
		exit(1);
	}

	ret = pthread_cond_init(&kg->teacher_out, NULL);
	if (ret) {
		perror_pthread(ret, "pthread_cond_init");
		exit(1);
	}
