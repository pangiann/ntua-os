...
{
  Node *tmp;
  tmp = find_node_with_id(id);
  if (tmp == q->rear) {
    perror("wrong id, try again");
    return -1;
  }
  if (tmp == NULL) {
    if (kill(q->front->pid, SIGKILL) < 0) {
  		perror("Sigkill signal error");
  		exit(1);
  	}
  }
  else {
	  if (kill(tmp->next->pid, SIGKILL) < 0) {
		   perror("Sigkill signal error");
		   exit(1);
	   }
  }
...
/* change to high priority after finding the node with the correct id */
/* one loop in queue needed */
...
sched_high_priority(int id)
{
  ...
  /* we found previous node because this will make our life easier when we'll change the queue */
  /* we need almost everytime prev node of the desired node in order to in order to move a node in queue */
  if (tmp == q->rear) {
    perror("wrong id, try again");
  }
  else if (( tmp != NULL && tmp->next->priority == 'h' ) || ( tmp == NULL && q->front->priority == 'h' ) ) {
    fprintf(stderr, "This process is already in high priority\n");
  }
  else {
    pid_t sh_pid = q->front->pid;
    change_to_high(q, tmp);
...
/* change to low priority after finding the node with the correct id */
/* one loop in queue needed */
...
sched_low_priority(int id)
{
  ...
  else if ((tmp != NULL && tmp->next->priority == 'l') || ( tmp == NULL && q->front->priority == 'l' )) {
    fprintf(stderr, "This process is already in low priority\n");
...
sched_create_task(char *executable)
{
    ...
    // we create the task and then parent process put it in queue
...
shell_request_loop(int request_fd, int return_fd)
{
	 ...
	 */
	for (;;) {
    // there is a process with high priority and shell is in low priority, do not receive requests from shell
    //if (q->rear_high != NULL && )
